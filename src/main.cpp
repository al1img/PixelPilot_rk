...
MPP_RET set_mpp_encoding_parameters(MppApi* mpi,  MppCtx ctx);
...
void init_buffer(MppFrame frame) {
	...
	if (encode.enable) {
		...
		ret = mpp_buffer_get(encode.frm_grp, &encode.pkt_buf, CODEC_ALIGN(hor_stride, 64) * CODEC_ALIGN(ver_stride, 64) * 2);
		assert(!ret);
		...
		ret = set_mpp_encoding_parameters(encode.mpi, encode.ctx);
		assert(!ret);
	}
...
void *__DISPLAY_THREAD__(void *param)
{
	...
	while (!frm_eos) {
		...
		if (encode.enable && encode.enable && encode.frm_grp) {
...
void *__ENCODE_THREAD__(void *param)
{
	...
	if (NULL == fp_output) {
		spdlog::error("failed to open output file %s\n", "test.mp4");
	}
	...
	while (!frm_eos) {
        ...
        mpp_packet_init_with_buffer(&packet, encode.pkt_buf);
		...
		do {
			...
			if (dvr_enabled && dvr != NULL) {
				...
				spdlog::debug("Packet len {}", len);

				dvr->frame(std::make_shared<std::vector<uint8_t>>((uint8_t*)ptr, (uint8_t*)ptr + len));
			}
...
void read_gstreamerpipe_stream(MppPacket *packet, int gst_udp_port, const char *sock ,const VideoCodec& codec){
    ...
    auto cb=[&packet,/*&decoder_stalled_count,*/ &bytes_received, &period_start](std::shared_ptr<std::vector<uint8_t>> frame){
        ...
        if (dvr_enabled && dvr != NULL && !encode.enable) {
			dvr->frame(frame);
        }
    };
...
int main(int argc, char **argv)
{
	...
	if (encode.enable) {
		...
		// Initialize encoder
		ret = mpp_check_support_format(MPP_CTX_ENC, encode.mpp_type);
		assert(!ret);
