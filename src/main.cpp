...
void init_buffer(MppFrame frame) {
	...
	if (encode.enable) {
		create_encode_buffers(frm_width, frm_height, hor_stride, ver_stride, fmt);
	}
...
void *__DISPLAY_THREAD__(void *param)
{
	...
	while (!frm_eos) {
		...
		if (encode.enable && encode.frm_grp) {
			ret = pthread_mutex_lock(&osd_mutex);
			assert(!ret);
			ret = copy_osd_buf(osd_bufs.bufs[osd_bufs.buf_switch]);
			assert(!ret);
...
int main(int argc, char **argv)
{
	...
	if (encode.enable) {
		init_encode(dvr, mpp_type, video_framerate);
	}
